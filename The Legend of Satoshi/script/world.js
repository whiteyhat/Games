var World = {
	...
	updateSupplies: function() {
		...
		for(var k in Path.outfit) {
			...
			if(num > 0 && item.length === 0) {
				...
				if(k == 'Lighting node' && World.water > 0) {
					item.insertAfter(water);
				} else if(k == 'Lighting node') {
	...
	useSupplies: function() {
		...
		if(World.foodMove >= movesPerFood) {
			World.foodMove = 0;
			var num = Path.outfit['Lighting node'];
			num--;
			if(num === 0) {
				Notifications.notify(World, _('the Lightning has run out'));
			} else if(num < 0) {
				// Starvation! Hooray!
				num = 0;
				if(!World.starvation) {
				...
				World.setHp(World.health + World.LightningHeal());
			}
			Path.outfit['Lighting node'] = num;
		}
		// Water
	...
	LightningHeal: function() {
		return World.Lightning_HEAL * ($SM.hasPerk('gasLightningome') ? 2 : 1);
	},
