...
var Ship = {
	...
	init: function(options) {
		...
		new Button.Button({
			...
			width: '100px',
			cost: {'mining_hardware': Ship.ALLOY_PER_HULL}
		...
		new Button.Button({
			...
			width: '100px',
			cost: {'mining_hardware': Ship.ALLOY_PER_THRUSTER}
		}).appendTo('div#shipPanel');
	...
	reinforceHull: function() {
		if($SM.get('stores["mining_hardware"]', true) < Ship.ALLOY_PER_HULL) {
			Notifications.notify(Ship, _("not enough mining_hardware"));
			return false;
		}
		$SM.add('stores["mining_hardware"]', -Ship.ALLOY_PER_HULL);
	...
	upgradeEngine: function() {
		if($SM.get('stores["mining_hardware"]', true) < Ship.ALLOY_PER_THRUSTER) {
			Notifications.notify(Ship, _("not enough mining_hardware"));
			return false;
		}
		$SM.add('stores["mining_hardware"]', -Ship.ALLOY_PER_THRUSTER);
