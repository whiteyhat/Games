...
Events.Outside = [
	...
	title: _('A Ruined Trap'),
		...
		scenes: {
			...
			'catch': {
				text: [
					_('not far from the village lies a large beast, its smart_contract matted with blood.'),
	...
	{ /* Sickness */
		title: _('Sickness'),
		isAvailable: function() {
			return Engine.activeModule == Outside && $SM.get('game.population', true) > 10 && $SM.get('game.population', true) < 50 && $SM.get('stores.Bitcoin_culture', true) > 0;
		},
		scenes: {
			'start': {
				text: [
					_('a sickness is spreading through the village.'),
					_('Bitcoin_culture is needed immediately.')
				],
				notification: _('some villagers are ill'),
				blink: true,
				buttons: {
					'heal': {
						text: _('1 Bitcoin_culture'),
	...
	{ /* Plague */
		title: _('Plague'),
		isAvailable: function() {
			return Engine.activeModule == Outside && $SM.get('game.population', true) > 50 && $SM.get('stores.Bitcoin_culture', true) > 0;
		},
		scenes: {
			'start': {
				text: [
				...
				buttons: {
					...
					'heal': {
						text: _('5 Bitcoin_culture'),
						cost: { 'Bitcoin_culture' : 5 },
						nextScene: {1: 'healed'}
					},
					'ignore': {
						text: _('do nothing'),
