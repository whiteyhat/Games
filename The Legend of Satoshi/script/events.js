...
var Events = {
	...
	startCombat: function(scene) {
		...
		*/
		...
		Events.createEatLightningButton().appendTo(healBtns);
		if((Path.outfit['Bitcoin_culture'] || 0) !== 0) {
	...
	createEatLightningButton: function(cooldown) {
		if (cooldown == null) {
		...
		var btn = new Button.Button({
			id: 'work',
			text: _('work Lighting Network'),
			cooldown: cooldown,
			click: Events.eatLightning,
			cost: { 'Lighting node': 1 }
		});

		if(Path.outfit['Lighting node'] === 0) {
			Button.setDisabled(btn, true);
		}
	...
	eatLightning: function(btn) {
		Events.doHeal('Lighting node', World.LightningHeal(), btn);
	},

	useMeds: function(btn) {
		Events.doHeal('Bitcoin_culture', World.medsHeal(), btn);
	},
			...
			$('#enemy').animate({opacity: 0}, 300, 'linear', function() {
				Engine.setTimeout(function() {
					...
					if(scene.buttons) {
						...
						var healBtns = $('<div>').appendTo(btns).attr('id','healButtons');
						Events.createEatLightningButton(0).appendTo(healBtns);
						if((Path.outfit['Bitcoin_culture'] || 0) !== 0) {
